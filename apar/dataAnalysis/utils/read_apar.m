function [dataOut] = read_apar(infile,indata)
...
%% Sort angles within sweeps
sweepMode=ncread(infile,'sweep_mode');
sweepMode=cellstr(sweepMode');

outVars=fields(data);

for kk=1:size(data,2)
    if strcmp(sweepMode{kk},'sector')
        [dataOut(kk).azimuth,inds]=sort(data(kk).azimuth);
        for ll=1:length(outVars)
            if ~strcmp(outVars{ll},'azimuth') & ~strcmp(outVars{ll},'range')
                dataOut(kk).(outVars{ll})=data(kk).(outVars{ll})(inds,:);
            end
        end
    elseif strcmp(sweepMode{kk},'rhi')
        [dataOut(kk).elevation,inds]=sort(data(kk).elevation);
        for ll=1:length(outVars)
            if ~strcmp(outVars{ll},'elevation') & ~strcmp(outVars{ll},'range')
                dataOut(kk).(outVars{ll})=data(kk).(outVars{ll})(inds,:);
            end
        end
    end
    dataOut(kk).range=data(kk).range;
    dataOut(kk).sweepMode=sweepMode{kk};
end

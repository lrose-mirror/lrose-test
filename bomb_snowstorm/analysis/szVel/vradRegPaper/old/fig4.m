...
close all
f1 = figure('Position',[200 500 1130 1000],'DefaultAxesFontSize',12,'visible',showPlot);
...
view(2);
title('(a) REG velocity censored for VRAD (m s^{-1})');
%xlabel('km');
ylabel('km');
...
view(2);
title('(c) VRAD-REG velocity (m s^{-1})');
xlabel('km');
ylabel('km');
...
view(2);
title('(d) VRAD-REG filled velocity (m s^{-1})');
xlabel('km');
%ylabel('km');
